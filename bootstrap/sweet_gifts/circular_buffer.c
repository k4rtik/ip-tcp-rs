 ...
 */
uint16_t circular_buffer_get_bytes_until_wrap(circular_buffer_t *cbuf,
                                              unsigned char *ptr);
...
int circular_buffer_write(circular_buffer_t *cbuf, const void *buf,
    ...
    int ret = -1;
    // Loop until we manually break out (when some data is written) -- skip if
    // count == 0
    while (count > 0) {
        ...
        if (!circular_buffer_is_full(cbuf)) {
            ...
            uint16_t available_capacity =
                circular_buffer_get_available_capacity(cbuf);
            ...
            uint16_t bytes_until_wrap =
                circular_buffer_get_bytes_until_wrap(cbuf, cbuf->write_pointer);
            ...
            if (bytes_until_wrap > length_to_write) {
                ...
                memcpy(cbuf->data, buf + bytes_until_wrap,
                       length_to_write - bytes_until_wrap);

                // Update pointer
                cbuf->write_pointer =
                    cbuf->data + (length_to_write - bytes_until_wrap);
            }

            // Update size, set return value, and wake up someone waiting to
            // read the data.
...
#ifdef CIRCULAR_BUFFER_DEBUG
            printf("Circular buffer sleeping as there is no room to write "
                   "right now!\n");
#endif
...
int circular_buffer_read(circular_buffer_t *cbuf, void *buf, size_t count) {
    ...
    int ret = -1;
    // Loop until we manually break out (when some data is read) -- skip if
    // count == 0
    while (count > 0) {
        ...
        if (!circular_buffer_is_empty(cbuf)) {
            ...
            uint16_t bytes_until_wrap =
                circular_buffer_get_bytes_until_wrap(cbuf, cbuf->read_pointer);
            ...
            if (bytes_until_wrap > length_to_read) {
...
#endif
                // Copy remaining bytes after we wrap (from beginning).
                memcpy(buf + bytes_until_wrap, cbuf->data,
                       length_to_read - bytes_until_wrap);
                ...
                // Update pointer
                cbuf->read_pointer =
                    cbuf->data + (length_to_read - bytes_until_wrap);
            }
